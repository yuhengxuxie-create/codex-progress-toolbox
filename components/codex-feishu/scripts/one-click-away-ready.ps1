...
$Stop = Invoke-ProgressCli -Context $Context -Command 'guardian-stop' -Arguments @('--timeout', '30')
...
& (Join-Path $PSScriptRoot 'disable-autostart.ps1') -ToolsRoot $ToolsRoot
if (-not $?) { throw '通信监督任务停用失败；不能报告安全准备完成。' }
$Report.Add('开机自启：已停用本安装通信监督任务。')
...
$Report.Add('')
$Report.Add('[结论]')
$Report.Add('离开前安全准备已完成：服务未运行、通信监督已停用。')
