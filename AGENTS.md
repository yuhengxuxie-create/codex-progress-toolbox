# Codex Feishu Ecosystem 1.6.1 安装指令
...
本目录对应正式版本 v1.6.1。v1.5.0 首次升级请使用 UPGRADE.md 的专用脚本，不要求旧版内置更新按钮。原应用与绑定保留；必须阅读 docs/FEISHU_UPGRADE_PERMISSIONS.md，按需补卡片回调 card.action.trigger 和菜单事件 application.bot.menu_v6。不要把权限 scope 未新增理解为无需检查平台配置。
